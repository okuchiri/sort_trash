...
TARGET_FILTER_ALPHA="${TARGET_FILTER_ALPHA:-0.20}"
TARGET_FILTER_RESET_DISTANCE_M="${TARGET_FILTER_RESET_DISTANCE_M:-0.08}"
...
Environment overrides:
  ...
  TARGET_FILTER_ALPHA, TARGET_FILTER_RESET_DISTANCE_M
...
case "$MODE" in
  ...
  fake-grasp)
    ...
    run_with_display scripts/control/run_fake_grasp_cycle.py \
      ...
      --calibration-file "$CALIBRATION_FILE" \
      --hover-height-m "$HOVER_HEIGHT_M" \
      --target-filter-alpha "$TARGET_FILTER_ALPHA" \
      --target-filter-reset-distance-m "$TARGET_FILTER_RESET_DISTANCE_M" \
      "${EXTRA_ARGS[@]}"
    ;;
  -h|--help|help)
    usage
